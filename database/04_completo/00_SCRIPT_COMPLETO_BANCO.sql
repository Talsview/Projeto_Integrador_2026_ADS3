-- =================================================================================================
-- SISTEMA DE GESTÃO PARA OFICINA MECÂNICA AV CAR AUTO CENTER
-- SCRIPT ÚNICO DO BANCO DE DADOS - POSTGRESQL
-- Versão consolidada: Etapa 21
-- =================================================================================================
-- Objetivo:
--   Executar em um único arquivo a criação do modelo físico e a carga inicial obrigatória.
--
-- Como usar no pgAdmin:
--   1. Criar o banco de dados: car_repair
--   2. Abrir o Query Tool no banco car_repair
--   3. Executar este arquivo inteiro
--   4. Rodar o backend com spring.jpa.hibernate.ddl-auto=validate
--
-- Observação importante:
--   Este script NÃO apaga dados existentes. Ele usa CREATE TABLE IF NOT EXISTS e INSERT ... ON CONFLICT.
--   Para recriar o banco do zero durante testes, apague e crie novamente o database car_repair no pgAdmin
--   e execute este arquivo outra vez.
--
-- Relação com os demais arquivos:
--   Este arquivo substitui, para execução prática, os arquivos 01_create_schema.sql e 02_seed_inicial.sql.
--   Os demais scripts 03, 04, 05... são apenas verificações técnicas opcionais das etapas.
-- =================================================================================================

BEGIN;

-- =================================================================================================
-- 1. MODELO FÍSICO - TABELAS, CHAVES, CONSTRAINTS E ÍNDICES
-- =================================================================================================
...
-- =================================================================================================
-- 2. DADOS INICIAIS OBRIGATÓRIOS PARA FUNCIONAMENTO DO SISTEMA
-- =================================================================================================
...
-- =================================================================================================
-- 3. CONSULTAS RÁPIDAS DE CONFERÊNCIA
-- =================================================================================================

SELECT 'status_ordem_servico' AS tabela, COUNT(*) AS total FROM status_ordem_servico;
SELECT 'funcao' AS tabela, COUNT(*) AS total FROM funcao;
SELECT 'marca' AS tabela, COUNT(*) AS total FROM marca;
SELECT 'modelo' AS tabela, COUNT(*) AS total FROM modelo;
SELECT 'servico' AS tabela, COUNT(*) AS total FROM servico;
